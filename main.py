...
def mediafire():
	if request.args.get('url'):
		try:
			...
			detail = soup.find_all('ul', {'class':'details'})
			filename = soup.find_all('div', {'class':'filename'})
			for fname in detail:
...
def blackpink():
	if request.args.get('text'):
		try:
			...
			driver.get("https://en.ephoto360.com/create-blackpink-logo-online-free-607.html")
			btn = driver.find_element_by_xpath('//*[@id="text-0"]')
			btn.send_keys(text)
			btn.send_keys(Keys.RETURN)
			time.sleep(10)
			images = driver.find_element_by_xpath('//*[@id="view-image-wrapper"]/div[1]/img')
			njay = images.get_attribute('src')
			return {
				'status': 200,
				'result': njay
			}
...
def bp():
	bpg = bs(get('https://www.y2mate.com/en62').text, 'html.parser').find('div', class_="text-center")
	img = bpg.findAll('h4')
	return {
		'status': 200,
		'map': img,
...
def wattpad():
	be = bs(get('https://www.wattpad.com/search/anjay').text, 'html.parser').find('div', id_="app-container")
	em = be.findAll('li')
	return {
		'status': 200,
		'title': em.text
